...
class AnnotationStorage:
    ...
    def _ensure_tables(self):
        ...
        try:
            ...
            """)
            # ⚡ OPTIMIZATION: Composite index for acc_score aggregation
            # Covers the classification dashboard's entity completeness check
            conn.execute("""
                CREATE INDEX IF NOT EXISTS idx_ner_ann_doc_layer_type
                ON ner_annotations(doc_id, layer, entity_type)
            """)

            # ⚡ OPTIMIZATION: ner_documents.candidate_id for JOIN performance
            # Used by annotation_tool queue, classification_dashboard, exports
            conn.execute("""
                CREATE INDEX IF NOT EXISTS idx_ner_doc_candidate
                ON ner_documents(candidate_id)
            """)

            # ⚡ OPTIMIZATION: ner_documents.status for WHERE clause filtering
            # Used by stats queries, queue filtering, export filtering
            conn.execute("""
                CREATE INDEX IF NOT EXISTS idx_ner_doc_status
                ON ner_documents(status)
            """)

            # ⚡ OPTIMIZATION: ner_annotations.updated_at for activity dashboard
            # The "recent 7 days" query uses DATE(updated_at) for grouping
            conn.execute("""
                CREATE INDEX IF NOT EXISTS idx_ner_ann_updated
                ON ner_annotations(updated_at)
            """)
            ...
            """)
            # ⚡ OPTIMIZATION: Composite index for IAA span lookups
            # _load_spans() queries both doc_id AND annotator_name together
            conn.execute("""
                CREATE INDEX IF NOT EXISTS idx_iaa_doc_annotator
                ON iaa_annotations(doc_id, annotator_name)
            """)

            # ⚡ OPTIMIZATION: iaa_annotations.candidate_id for JOIN performance
            conn.execute("""
                CREATE INDEX IF NOT EXISTS idx_iaa_candidate
                ON iaa_annotations(candidate_id)
            """)
